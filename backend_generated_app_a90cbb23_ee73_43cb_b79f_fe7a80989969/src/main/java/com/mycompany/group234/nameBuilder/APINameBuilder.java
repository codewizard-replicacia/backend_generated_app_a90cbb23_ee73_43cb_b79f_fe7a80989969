...
public class APINameBuilder implements JPAEdmNameBuilder {
	...
	public String buildEntitySetName(final String entityTypeName) {
    	...
    	eSetName = "CreatingBudget".equals(entityTypeName) ? "CreatingBudgets" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "Casting".equals(entityTypeName) ? "Castings" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "Crew".equals(entityTypeName) ? "Crews" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "OrganisingLogistics".equals(entityTypeName) ? "OrganisingLogistics" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "ScoutingLocations".equals(entityTypeName) ? "ScoutingLocations" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "ProducerRaiseFunds".equals(entityTypeName) ? "ProducerRaiseFunds" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "PreProduction".equals(entityTypeName) ? "PreProductions" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "ProductionActualFilming".equals(entityTypeName) ? "ProductionActualFilmings" : defaultNameBuilder.buildEntitySetName(entityTypeName);
        return eSetName;
    }
